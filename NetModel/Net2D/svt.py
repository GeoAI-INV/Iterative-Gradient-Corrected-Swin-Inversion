...
class ClassAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads):
        ...
        self.kv = nn.Linear(dim, dim * 2)  #create key and value so the outpout is 2*dim
        self.q = nn.Linear(dim, dim)       #create query
        self.proj = nn.Linear(dim, dim)    #create project 投影
    ...
    def forward(self, x):
        B, N, C = x.shape
        kv = self.kv(x).reshape(B, N, 2, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)# 拆分成多(num_heads)头
        k, v = kv[0], kv[1]
        q = self.q(x[:, :1, :]).reshape(B, self.num_heads, 1, self.head_dim)# only the first elements
...
class FFN(nn.Module):
    "full connection the last dim is channel (N,H*W,C->hidden->C)  MLP"
...
class SVT(nn.Module):
    def __init__(self, 
        ...
        **kwargs
        ...
        for i in range(num_stages):
            if i == 0:
                patch_embed = Stem(self.in_chans, stem_hidden_dim, embed_dims[i])
            else:
    ...
    def forward(self, x):
        if not self.return_dense:       #when self.return is False 
...
if __name__ == '__main__':
    ...
    block =Block(
        ...
        norm_layer=norm_layer,
        sr_ratio = 4,
        block_type='scatter' if 0 < 1 else 'std_att')
    
    
    xfm = DTCWTForward(J=1, biort='near_sym_b', qshift='qshift_b')
    # torch.cuda.empty_cache()
    net = svt_s(token_label=False)
