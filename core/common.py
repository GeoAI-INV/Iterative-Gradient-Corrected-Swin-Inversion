...
# ----------------------------------------------------------------------------------------------------------------------
def DataProess_v2(data, args):
    patch_data = []
    patch_counts = []
    imgsize = []
    maxdata = []
    for x in data:
        maxtemp = np.amax(abs(x))
        maxdata.append(maxtemp)
        x = x / maxtemp
        imgsize.append(x.shape)  
        patch = np.expand_dims(get_patches(x, args.patchsize, args.stride), 1)
        patch_data.append(patch)
        patch_counts.append(patch.shape[0]) 

    maxdata = [value for value, count in zip(maxdata, patch_counts) for _ in range(count)]
    scale = []
    for x in maxdata:
        scale.append(np.full(args.patchsize, x))
    scale = np.expand_dims(np.array(scale), 1)

    patch_data = np.concatenate(patch_data, axis=0)
    patch_data = torch.tensor(patch_data).to(device).type(torch.float32)
    scale = torch.tensor(scale).to(device).type(torch.float32)

    return patch_data, patch_counts, imgsize, scale


# ----------------------------------------------------------------------------------------------------------------------
def DataBack(patch_data, patch_counts, imgsize, args):
    start_idx = 0
    m_rec = []
    for count in patch_counts:
        end_idx = start_idx + count
        restored_element = patch_data[start_idx:end_idx]  # 取出一个图片的切块
        m_rec.append(restored_element)
        start_idx = end_idx
    data = []
    for x, size in zip(m_rec, imgsize):
        data.append(back_image(x.squeeze(), size, args.stride))
    return data


# ----------------------------------------------------------------------------------------------------------------------
def get_pretrain_data(args, test=False):
    # Loading data
    logimp, implow, syn_train, syn_noise, implog_train, Masks, imp_test, syn_test, syn_test_noise, low_test = LoadData_v2()
    assert implog_train[0].shape[1] == syn_train[0].shape[
        1], 'Number of traces is not consistent. Got {} traces for seismic data and {} traces for acoustic acoustic impedance'.format(
        implog_train[0].shape[1], syn_train[0].shape[1])
    dataset_size = len(syn_train)
    train_size = int(0.8 * dataset_size)
    indices = np.arange(dataset_size)
    train_indices = indices[(np.linspace(0, len(indices) - 1, train_size)).astype(int)]  # 均匀抽取
    val_indices = np.setdiff1d(indices, train_indices)

    train_dataset = [syn_train[i] for i in train_indices]
    val_dataset = [syn_train[i] for i in val_indices]

    train_syn, train_counts, train_imgsize, train_max = DataProess_v2(train_dataset, args)
    val_syn, val_counts, val_imgsize, val_max = DataProess_v2(val_dataset, args)

    if not test:
        train_loader = DataLoader(train_syn, batch_size=args.batch_size, shuffle=False)
        return train_loader, train_counts, train_imgsize, train_max
    else:
        test_loader = DataLoader(val_syn, batch_size=args.batch_size, shuffle=False, drop_last=False)
        return test_loader, val_counts, val_imgsize, val_max


# ----------------------------------------------------------------------------------------------------------------------
def get_data_v2(args, test=False):
    # Loading data noise_flag
    real_imp, implow, syn_train, syn_noise, implog_train, Masks, imp_test, syn_test, syn_test_noise, low_test = LoadData_v2()
    assert implog_train[0].shape[1] == syn_train[0].shape[
        1], 'Number of traces is not consistent. Got {} traces for seismic data and {} traces for acoustic acoustic impedance'.format(
        implog_train[0].shape[1], syn_train[0].shape[1])

    synblock = forwardblock(implog_train[0], f0=30, dt=0.001, wave_len=51, snr_db=10)

    if not test:
        patch_real, real_counts, imgsize, real_imp_max = DataProess_v2(real_imp, args)
        patch_syn, syn_counts, imgsize, syn_max = DataProess_v2(syn_train, args)
        patch_syn_n, syn_counts_n, imgsize, syn_max_n = DataProess_v2(syn_noise, args)
        patch_implow, implow_counts, imgsize, implow_max = DataProess_v2(implow, args)
        patch_logimp, logimp_counts, imgsize, logimp_max = DataProess_v2(implog_train, args)
        patch_labels, _, _, _ = DataProess_v2(Masks, args)

        train_loader = DataLoader(
            TensorDataset(patch_syn_n, patch_syn, patch_logimp, patch_implow, patch_labels, real_imp_max / syn_max),
            batch_size=args.batch_size, shuffle=False)  # 保持数据原始顺序
        return train_loader, synblock.wavelet(), logimp_max, implow_max, syn_counts, imgsize
    else:

        test_imp, imp_counts, size_3d, imp_max = DataProess_v2(imp_test, args)
        test_syn3d, syn3d_counts, size_3d, syn3d_max = DataProess_v2(syn_test, args)
        test_syn3d_n, syn3d_counts_n, size_3d, syn3d_max_n = DataProess_v2(syn_test_noise, args)
        test_low, low_counts, size_3d, low_max = DataProess_v2(low_test, args)

        test_loader = DataLoader(
            TensorDataset(test_syn3d_n, test_syn3d, test_imp, test_low, imp_max / syn3d_max),
            batch_size=args.batch_size,
            shuffle=False, drop_last=False)

        return test_loader, synblock.wavelet(), imp_max, low_max, syn3d_counts, size_3d
...
def get_data_temp(args, test=False):
    # Loading data
    if args.data_flag == 'model':
        print('model')
        logimp, implow, synblock, labels, cfs, cfs_wavelets, angle = LoadData(args)
        wav = synblock.wavelet()
        if args.use_noise:
            syn = synblock.noisesyn()
            print('noise')
        else:
            syn = synblock.cleansyn()
            print('clean')
        ...
        print('field')
        logimp, implow, syn, labels, cfs, cfs_wavelets, angle, wav = LoadData_field(args)
    ...
    patch_syn = syn / np.max(abs(syn))
    patch_logimp = logimp / np.max(abs(logimp))
    patch_implow = implow / np.max(abs(implow))

    patch_labels = torch.tensor(labels).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32)

    if not test:
        train_loader = DataLoader(
            TensorDataset(torch.tensor(patch_syn).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32),
                          torch.tensor(patch_logimp).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32),
                          torch.tensor(patch_implow).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32),
                          patch_labels),
            batch_size=args.batch_size, shuffle=False)  # Batch size is 1
        return train_loader, wav, cfs_wavelets, m_min_max[1] / s_min_max[1], m_min_max[1] / cfs_min_max[1], m_min_max[1] / high_min_max[
            1], logimp.shape
    else:
        test_loader = DataLoader(
            TensorDataset(torch.tensor(patch_syn).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32),
                          torch.tensor(patch_logimp).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32),
                          torch.tensor(patch_implow).unsqueeze(0).unsqueeze(0).to(device).type(torch.float32)),
            batch_size=args.batch_size, shuffle=False, drop_last=False)  # Batch size is 1
        return test_loader, wav, m_min_max, mlow_min_max, s_min_max, logimp.shape
